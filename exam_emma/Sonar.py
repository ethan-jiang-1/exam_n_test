...
def getNewBoard():
    ...
    for x in range(60): #The main list is a list of 60 lists
        board.append([])
        for y in range(15): #Each list in the main lsit has 15 single-character strings
...
def drawBoard(board):
    ...
    for row in range(15):
        # Single-digit numbers needed to be padded with an extra space
        if row <10:
            extraSpace = ' '
        else:
            extraSpace = ''
...
def getRandomChests(numChests):
    ...
    while len(chests) < numChests:
        ...
        if newChest not in chests: #Make sure a chest is ot already here
            chests.append(newChest)
    return chests

def isOnBoard(x,y):
    #Return True the coordinate are on the board; otherwise, return False
...
def showInstructions():
    print('''Instructions: 
You are the caption of Simon, a treasure-hunting ship. Your current mission
is to use sonar devices to find three sunken treasure chests at the bottom of 
the ocean. But you only have a cheap sonar that finds distance, not direction.
...
while True:
    ...
    while sonarDevices > 0:
        ...
        if moveResult == False:
            ...
            if moveResult == 'You have found a sunken treasrure chest!':
